...
def _normalize_manual_item_image(data: bytes) -> Image.Image:
    ...
    # If the user's square screenshot still contains a large neutral margin,
    # locate the colourful SSR/SR card border and tighten to that square first.
    rgb = image.convert("RGB")
    mask = Image.new("L", rgb.size)
    mask.putdata(
        [
            255 if max(pixel) > 100 and max(pixel) - min(pixel) > 28 else 0
            for pixel in rgb.getdata()
        ]
    )
    colour_box = mask.getbbox()
    if colour_box:
        left, top, right, bottom = colour_box
        box_width = right - left
        box_height = bottom - top
        if box_width >= 20 and box_height >= 20 and 0.72 <= box_width / box_height <= 1.38:
